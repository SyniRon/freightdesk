...
- **RouteFormula.** A discriminated union describing how to compute the reward for a given route. Five kinds:
  ...
  - **`clamped-rate`** — `clamp(vol × ratePerM3, floor, fullLoad)`, then `max`'d against `collateral × collateralPct` when the optional `collateralPct` is present (a leg without one has no collateral component at all). `fullLoad` is the reward at a full load, i.e. `ratePerM3 × maxVol`. `floor` is a route-level lower bound on the volume reward — see `minReward`.
- **`rushFee`.** Optional per-route addend toggled by a UI switch. ADFU is currently 250 M ISK per rush contract on each route.
- **`minReward`.** Service-level floor, applied outside the formula. If the computed reward is below `minReward`, the user pays the floor — and FreightDesk surfaces an amber info banner that shows both numbers explicitly so the gap isn't hidden. Guard: `minReward > 0 && formulaResult < minReward`. `clamped-rate` is the exception to where the floor lives: it floors itself *inside* the formula, at its own route-level `floor`, so that is the operative floor for such a route. The outer `minReward` still wraps the result but only bites when it exceeds `floor`, and the "too small" advisory compares against `floor` for that kind.
