...
- **RouteFormula.** A discriminated union describing how to compute the reward for a given route. Five kinds:
  ...
  - **`clamped-rate`** — `clamp(vol × ratePerM3, floor, fullLoad)`, then `max`'d against `collateral × collateralPct` when the optional `collateralPct` is present. `fullLoad` is the reward at a full load (typically `ratePerM3 × maxVol`); `floor` is the route-level lower bound, and it — not `minReward` — is this kind's operative floor.
- **`rushFee`.** Optional per-route addend toggled by a UI switch. ADFU is currently 250 M ISK per rush contract on each route.
- **`minReward`.** Reward floor, resolved route-first: a `ServiceRoute` `minReward` **replaces** the `Service` value rather than stacking with it, and the `Service` value is the default for routes that set none. With neither set there is no floor. Applied outside the formula: if the computed reward is below the resolved floor, the user pays the floor — and FreightDesk surfaces an amber info banner that shows both numbers explicitly so the gap isn't hidden. Guard: `minReward > 0 && formulaResult < minReward`. `clamped-rate` is the exception to where the floor lives: it floors itself *inside* the formula, at its own route-level `floor`, so that is the operative floor for such a route. The outer `minReward` still wraps the result but only bites when it exceeds `floor`.
- **`maxVol` / `maxCollateral`.** Volume and collateral caps, resolved route-first exactly as `minReward` is: a `ServiceRoute` value **replaces** the `Service` default rather than stacking with it. `maxCollateral` alone has a third source above both — a positive `QuoteOverrides.maxCollateral` displaces route and service caps alike, and can impose a cap on a service that publishes none. Nothing in the UI sets it today: the settings drawer's direct overrides are collateral, volume and rate, so the cap override is an evaluator-level seam only tests reach. A load exceeding a cap is not unserviceable — the service will still take it as multiple contracts. This is the **splittable** state (see *Eligibility*), not an error.
- **Rates verification (`ratesVerified`).** The date a service's configured rates were last checked against that shipper's *published rate card* — the shipper's own calculator or price list, which is authoritative. A check that finds nothing changed is still a verification and still bumps the date; that is the common case, and it is why the date is declared in the YAML rather than inferred from the config's edit history (a no-change check leaves no commit, and an unrelated typo fix would otherwise read as a fresh check). Required on every service config, must be a real `YYYY-MM-DD` calendar date, and must not be in the future — a mistyped year would silently suppress that service's staleness signal forever. Surfaces as `Service.updated`, rendered on the card as **Rates verified**, and wears an amber `stale` tag past `STALE_AFTER_DAYS`. ADR 0015.
  _Avoid_: rates updated, last edited, last modified
...
- **`Service.contract` metadata.** Per-service block holding `expiration` and `daysToComplete` (both required), plus optional `descriptionHint` and `source`. Rendered as an info panel beneath the copy grid because those EVE fields are UI dropdowns the user has to set manually — they aren't paste targets. `source` is the odd one out: it is the shipper's published rate-card URL, provenance for the config rather than a Create Contract dialog field.
- **Custom service.** A synthetic `Service` generated at runtime from user input, not from a catalog YAML. It appears **only when no catalog Service matches the picked route** (the no-route case) and exists to price a route the catalog doesn't cover. The user sets a per-m³ rate and an optional collateral-percent; those build a real `RouteFormula` (`rate-only`, or `max` when both are given) so the quote flows through the *same* `evaluateServices` / copy-block pipeline as any catalog service. Carries no `maxVol` / `maxCollateral` / `minReward` — no shipper imposes caps or a floor, so it is always `eligible`, never `splittable`. Its rate is owned by the card (its own `localStorage` key), independent of the global rate override (which displaces a *catalog* route's published rate and is inert when no route matches). Decoupled from the catalog couriers — never a phantom price painted onto a courier that declined the route.
  _Avoid_: manual rate card, phantom service, fallback quote
- **Recipient (custom-service Shipper).** The contract **assignee** — the exact in-game corp/player string pasted into EVE's "Recipient / Issue To" field (the copy block's *Shipper* row). For a catalog service it is `service.name`; for a custom service the user picks it from a combobox seeded with catalog corp strings or free-types it. **Optional** — empty means a *public contract* (no assignee, any hauler can accept). Seeding with catalog names exists chiefly to cover **catalog lag**: a known shipper serves a route in-game before its YAML is updated, so the user can still pick that shipper and copy its exact corp string today.
...
- **`web/src/lib/services.generated.ts`.** Typed service config emitted from the YAMLs in `web/services/` by `scripts/build-services.ts`. Every value comes from the config files — `updated` is each service's declared `ratesVerified`, so the output is identical on a laptop, in CI, and inside the image. Gitignored.
