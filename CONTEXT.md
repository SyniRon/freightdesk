...
- **`minReward`.** Service-level floor, applied outside the formula. If the computed reward is below `minReward`, the user pays the floor — and FreightDesk surfaces an amber info banner that shows both numbers explicitly so the gap isn't hidden. Guard: `minReward > 0 && formulaResult < minReward`. `clamped-rate` is the exception to where the floor lives: it floors itself *inside* the formula, at its own route-level `floor`, so that is the operative floor for such a route. The outer `minReward` still wraps the result but only bites when it exceeds `floor`.
